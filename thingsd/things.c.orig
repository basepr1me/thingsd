...
things_dispatch_parent(int fd, struct privsep_proc *p, struct imsg *imsg)
{
	...
	case IMSG_ADD_THING:
		...
		memcpy(&thing->iface, nti.iface, sizeof(thing->iface));

		memcpy(&thing->ipaddr, nti.ipaddr, sizeof(thing->ipaddr));

		memcpy(&thing->parity, nti.parity, sizeof(thing->parity));

		memcpy(&thing->name, nti.name, sizeof(thing->name));

		memcpy(&thing->password, nti.password,
		    sizeof(thing->password));

		memcpy(&thing->location, nti.location,
		    sizeof(thing->location));

		memcpy(&thing->udp, nti.udp, sizeof(thing->udp));
		...
		thing->tls = nti.tls;
		memcpy(&thing->tls_cert_file, nti.tls_cert_file,
		    sizeof(thing->tls_cert_file));

		memcpy(&thing->tls_key_file, nti.tls_key_file,
		    sizeof(thing->tls_key_file));

		memcpy(&thing->tls_ca_file, nti.tls_ca_file,
		    sizeof(thing->tls_ca_file));

		memcpy(&thing->tls_crl_file, nti.tls_crl_file,
		    sizeof(thing->tls_crl_file));

		memcpy(&thing->tls_ocsp_staple_file,
